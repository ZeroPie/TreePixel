...
const goToSignUp = async () => {
  ...
  if (!user.value) {
    navigateTo(`/auth/sign-up?username=${username.value}`)
    loading.value = false
  }
  else {
    const websiteInfo = [{
    ...
    let { error } = await supabase.from('website').insert(websiteInfo as never[])
    if(!error)
    {
      const url = `${window.location.origin}/private/edit?link=${username.value}`
...
<template>
  ...
  <div class="middleSectionBox homeMiddle">
    <div class="container">
      <div class="linkTogetherSection">
        <div class="togetherLeftImgBox">
          <img src="/images/leftImg.webp" alt="" />
        </div>
        <div class="togetherMiddleBox">
          <h2>Link it all <span>Together.</span></h2>
          <p class="font24 mb0">Claim your unique link name, stand<br> out from the crowd!</p>
          <form @submit.prevent="goToSignUp()" class="cliamSection">
            <div class="homeLinkInput input-box">
              <span>treepixel.app/</span>
              <input type="text" placeholder="" v-model="username">
            </div>
            <button type="submit" class="claimBtn">Claim your link</button>
            <span v-if="inValidUserName" class="text-white">{{ userNameValidationMessage }}</span>
          </form>
        </div>
        <!-- <div class="togetherRightImgBox">
          <img src="/images/rightImg.webp" alt="" />
        </div> -->
        <div class="position-relative">
          <div class="togetherRightImgBox">
            <img src="/images/rightImg-back.png" alt="" />
          </div>
          <img src="/images/phone-img-right.svg" alt="" class="phoneImgRight" />
        </div>
      </div>
    </div>
  </div>
